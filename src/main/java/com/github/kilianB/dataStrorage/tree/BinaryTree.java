...
/**
 * A not thread safe binary tree implementation used to quickly compute the <a
 * href="https://en.wikipedia.org/wiki/Hamming_distance">hamming distance</a> of
 * multiple hashes. The tree can be used to keep all hashes in memory, if a
 * persistent storage is required take a look at the database examples. <p>
 * 
 * To keep storage space minimal the tree is lazily populated and creates nodes
 * on the fly only if the node is required to represent a hash. </p>
 * 
 * <b>Example </b>
 * 
 * Hash(1011011) <ul> <li>The left child of a node represents a 1 bit</li>
 * <li>The right child of a node represents a 0 bit</li> <li>Padding bit 1 is
 * ignored as it's the same</li> </ul>
 * 
 * <pre>
 *       root
 *              0
...
public class BinaryTree<T> {
	...
	/**
	 * The root node of the tree representing the 1 padding pit.
	 */
	private Node root;
	...
	/**
	 * 
	 * @param ensureHashConsistency If true adding and matching hashes will 
	 * 	check weather they are generated by the same algorithms as the first hash added to the tree
	 * 
	 */
	 ...
	 * algorithms will return useable results. <p>
	 * 
	 ...
	 * 
	 * @param hash
	 *            The hash used to save the value in the tree
	 * @param value
	 *            The value which will be returned if the hash . Common values point
	 *            to the image used to create the hash or an id in a SQL table
	 * 
	 */
	 ...
	 * 
	 *  If the tree is configured to ensureHashConsistency this function will throw 
	 * an unchecked IlleglStateException if the checked hash does not comply with the
	 * first hash added to the tree.
	 * 
	 * @param hash
	 *            The hash to search for
	 * @param maxDistance
	 *            The maximal hamming distance deviation all found hashes may
	 *            possess. A distance of 0 will return all objects added whose hash
	 *            is exactly the hash supplied as the first argument
	 * 
	...
	public PriorityQueue<Result<T>> getElementsWithinHemmingDistance(Hash hash, int maxDistance) {
		...
		BigInteger hashValue = hash.getHashValue();
		int treeDepth = hashValue.bitLength() - 1;

		ArrayDeque<NodeInfo> queue = new ArrayDeque<>();

		// Skip the padding bit
		queue.add(new NodeInfo(root, 0, treeDepth, "1"));

		while (!queue.isEmpty()) {

			NodeInfo info = queue.poll();

			// System.out.printf("%-8s Depth: %d Distance: %d Next Bit: %s
			// %n",info.curPath,info.depth,info.distance,hashValue.testBit(info.depth-1)?
			// "1" : "0");
